...
MU_Modem_Error MU_Modem::begin(Stream &pUart, MU_Modem_FrequencyModel frequencyModel, MU_Modem_AsyncCallback pCallback)
{
    ...
    MU_DEBUG_PRINTLN("[MU_Modem] begin: Performing SoftReset...");
    ...
    MU_DEBUG_PRINTLN("[MU_Modem] begin: Delaying after reset...");
    delay(150); // Recommended delay after software reset


    MU_DEBUG_PRINTLN("[MU_Modem] begin: Enabling RSSI reporting...");
...
MU_Modem_Error MU_Modem::SetAddRssiValue()
{
    ...
    {
        MU_DEBUG_PRINTLN("[MU_Modem] SetAddRssiValue: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] SetAddRssiValue: Sending command...");
    m_WriteString(F(MU_SET_ADD_RSSI_STRING)); // Sends "@SION\r\n"
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] SetAddRssiValue: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SetAddRssiValue: m_WaitCmdResponse returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;
    ...
    if (m_rxIdx != MU_SET_ADD_RSSI_RESPONSE_LEN || strncmp(MU_SET_ADD_RSSI_RESPONSE, (char*)m_rxMessage, MU_SET_ADD_RSSI_RESPONSE_LEN) != 0) {
         MU_DEBUG_PRINTF("[MU_Modem] SetAddRssiValue: Unexpected response '%.*s'\n", m_rxIdx, m_rxMessage);
         rv = MU_Modem_Error::Fail; // Treat unexpected response as failure
    } else {
         MU_DEBUG_PRINTLN("[MU_Modem] SetAddRssiValue: OK (*SI=ON)");
    }
...
MU_Modem_Error MU_Modem::SetAutoReplyRoute(bool enabled, bool saveValue)
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
         MU_DEBUG_PRINTLN("[MU_Modem] SetAutoReplyRoute: Busy");
        return MU_Modem_Error::Busy;
    }
    ...
    MU_DEBUG_PRINTF("[MU_Modem] SetAutoReplyRoute: Sending command '%s'...", cmdBuffer);
    m_WriteString(cmdBuffer);
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] SetAutoReplyRoute: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SetAutoReplyRoute: m_WaitCmdResponse returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;

    if (saveValue) {
        rv = m_HandleMessage_WR();
        if (rv != MU_Modem_Error::Ok) return MU_Modem_Error::Fail;
    ...
    if (m_rxIdx != expectedLen || strncmp(expectedResponse, (char*)m_rxMessage, expectedLen) != 0) {
         MU_DEBUG_PRINTF("[MU_Modem] SetAutoReplyRoute: Unexpected response '%.*s'\n", m_rxIdx, m_rxMessage);
         rv = MU_Modem_Error::Fail;
    } else {
         MU_DEBUG_PRINTF("[MU_Modem] SetAutoReplyRoute: OK (*RR=%s)\n", enabled ? "ON" : "OFF");
    }
...
MU_Modem_Error MU_Modem::SoftReset()
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
         MU_DEBUG_PRINTLN("[MU_Modem] SoftReset: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] SoftReset: Sending command...");
    m_WriteString(F(MU_SET_SOFT_RESET_STRING)); // Sends "@SR\r\n"
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] SoftReset: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SoftReset: m_WaitCmdResponse returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;
    ...
    if (m_rxIdx != MU_SET_SOFT_RESET_RESPONSE_LEN || strncmp(MU_SET_SOFT_RESET_RESPONSE, (char*)m_rxMessage, MU_SET_SOFT_RESET_RESPONSE_LEN) != 0) {
         MU_DEBUG_PRINTF("[MU_Modem] SoftReset: Unexpected response '%.*s'\n", m_rxIdx, m_rxMessage);
         rv = MU_Modem_Error::Fail; // Treat unexpected response as failure
    } else {
         MU_DEBUG_PRINTLN("[MU_Modem] SoftReset: OK (*SR=00)");
    }
...
{
    if (!pEnabled) return MU_Modem_Error::InvalidArg;
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle) return MU_Modem_Error::Busy;

    MU_DEBUG_PRINT("[MU_Modem] GetAutoReplyRoute: Sending command...");
    m_WriteString(F(MU_GET_USR_ROUTE_STRING)); // @RR
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] GetAutoReplyRoute: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] GetAutoReplyRoute: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv == MU_Modem_Error::Ok) {
        MU_DEBUG_PRINT("[MU_Modem] GetAutoReplyRoute: Handling response...");
        if (m_rxIdx == MU_SET_USR_ROUTE_ON_RESPONSE_LEN && strncmp(MU_SET_USR_ROUTE_ON_RESPONSE, (char*)m_rxMessage, MU_SET_USR_ROUTE_ON_RESPONSE_LEN) == 0) {
            ...
            *pEnabled = false;
            MU_DEBUG_PRINTLN(" Result: OFF");
        } else {
            MU_DEBUG_PRINTF("[MU_Modem] GetAutoReplyRoute: Unexpected response '%.*s'\n", m_rxIdx, m_rxMessage);
            rv = MU_Modem_Error::Fail;
        }
    }
...
MU_Modem_Error MU_Modem::SetRouteInfoAddMode(bool enabled, bool saveValue)
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfoAddMode: Busy");
        return MU_Modem_Error::Busy;
    }
    ...
    MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfoAddMode: Sending command '%s'...", cmdBuffer);
    m_WriteString(cmdBuffer);
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfoAddMode: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfoAddMode: m_WaitCmdResponse returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;
    ...
    if (m_rxIdx != expectedLen || strncmp(expectedResponse, (char*)m_rxMessage, expectedLen) != 0) {
        MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfoAddMode: Unexpected response '%.*s'\n", m_rxIdx, m_rxMessage);
        rv = MU_Modem_Error::Fail;
    } else {
        MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfoAddMode: OK (*RI=%s)\n", enabled ? "ON" : "OFF");
    }
...
MU_Modem_Error MU_Modem::GetRouteInfoAddMode(bool* pEnabled)
{
    ...
    MU_DEBUG_PRINT("[MU_Modem] GetRouteInfoAddMode: Sending command...");
    m_WriteString(F(MU_GET_ROUTE_INFO_ADD_MODE_STRING)); // @RI
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] GetRouteInfoAddMode: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] GetRouteInfoAddMode: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv == MU_Modem_Error::Ok) {
        MU_DEBUG_PRINT("[MU_Modem] GetRouteInfoAddMode: Handling response...");
        if (m_rxIdx == MU_SET_ROUTE_INFO_ADD_MODE_ON_RESPONSE_LEN && strncmp(MU_SET_ROUTE_INFO_ADD_MODE_ON_RESPONSE, (char*)m_rxMessage, MU_SET_ROUTE_INFO_ADD_MODE_ON_RESPONSE_LEN) == 0) {
            ...
            *pEnabled = false;
            ...
            MU_DEBUG_PRINTF("[MU_Modem] GetRouteInfoAddMode: Unexpected response '%.*s'\n", m_rxIdx, m_rxMessage);
            rv = MU_Modem_Error::Fail;
        }
    }
...
MU_Modem_Error MU_Modem::TransmitData(const uint8_t *pMsg, uint8_t len, bool useRouteRegister)
{
     if (!pMsg || len == 0 || len > MU_MAX_PAYLOAD_LEN) {
        MU_DEBUG_PRINTF("[MU_Modem] TransmitData: Invalid args (pMsg=%p, len=%u)\n", pMsg, len);
        return MU_Modem_Error::InvalidArg;
    }

    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] TransmitData: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] TransmitData: Sending command preamble...");
    ...
    if (useRouteRegister) {
        m_WriteString(F("/R\r\n"));
        MU_DEBUG_PRINTF(" Payload (%u bytes) with /R option Sent.\n", len);
    } else {
        m_WriteString(F("\r\n")); // End command without options
        MU_DEBUG_PRINTF(" Payload (%u bytes) Sent.\n", len);
    }

    // --- The following *DT response and LBT (*IR) check remain as is ---
    MU_DEBUG_PRINTLN("[MU_Modem] TransmitData: Waiting for *DT response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] TransmitData: m_WaitCmdResponse (DT) returned %d\n", (int)rv);

    if (rv != MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] TransmitData: Failed waiting for *DT response.");
        return rv; // Return Timeout or Parse Error
    }

    // Check transmission response "*DT=XX"
    uint8_t transmissionResponseLen{};
    MU_DEBUG_PRINT("[MU_Modem] TransmitData: Checking *DT response...");
    rv = m_HandleMessageHexByte(&transmissionResponseLen, MU_TRANSMISSION_RESPONSE_LEN, MU_TRANSMISSION_RESPONSE_PREFIX);

    if (rv != MU_Modem_Error::Ok) {
         MU_DEBUG_PRINTF(" Failed. err=%d. Rx: '%.*s'\n", (int)rv, m_rxIdx, m_rxMessage); // This format is fine as it's part of a line
    ...
    {
         MU_DEBUG_PRINTF("[MU_Modem] TransmitData: Length mismatch in *DT response. Sent %u, Acked %u\n", len, transmissionResponseLen);
        rv = MU_Modem_Error::Fail; // Indicate failure due to length mismatch
        return rv;
    }
    MU_DEBUG_PRINTF(" *DT response OK (len=%u).\n", transmissionResponseLen);

    MU_DEBUG_PRINTLN("[MU_Modem] TransmitData: Waiting for LBT result...");
    ...
    if (lbt_check_rv == MU_Modem_Error::Ok) {
            ...
            strncmp(MU_INFORMATION_RESPONSE_PREFIX, (char*)m_rxMessage, strlen(MU_INFORMATION_RESPONSE_PREFIX)) == 0)
        {
            ...
            if (s_ParseHex(&m_rxMessage[strlen(MU_INFORMATION_RESPONSE_PREFIX)], 2, &infoCode) && infoCode == MU_INFORMATION_RESPONSE_ERR_NO_TX) {
                MU_DEBUG_PRINTLN("[MU_Modem] TransmitData: LBT failed (*IR=01).");
                return MU_Modem_Error::FailLbt;
            }
        }
        // Unexpected response other than *IR=01
        MU_DEBUG_PRINTF("[MU_Modem] TransmitData: Unexpected response during LBT check '%.*s'.\n", m_rxIdx, m_rxMessage);
    ...
    MU_DEBUG_PRINTLN("[MU_Modem] TransmitData: Transmission assumed successful.");
...
{
    if (!pMsg || len == 0 || len > MU_MAX_PAYLOAD_LEN) {
        MU_DEBUG_PRINTF("[MU_Modem] TransmitDataFireAndForget: Invalid args (pMsg=%p, len=%u)\n", pMsg, len);
        return MU_Modem_Error::InvalidArg;
    }

    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataFireAndForget: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] TransmitDataFireAndForget: Sending command preamble...");
    m_WriteString(F(MU_TRANSMISSION_PREFIX_STRING)); // "@DT"
    char lenStr[3];
    snprintf(lenStr, sizeof(lenStr), "%02X", len);
    m_WriteString(lenStr);
    ...
    MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataFireAndForget: Waiting for *DT response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] TransmitDataFireAndForget: m_WaitCmdResponse (DT) returned %d\n", (int)rv);

    if (rv != MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataFireAndForget: Failed waiting for *DT response.");
        return rv; // Return Timeout or Parse Error
    }

    // Check transmission response "*DT=XX"
    uint8_t transmissionResponseLen{};
    MU_DEBUG_PRINT("[MU_Modem] TransmitDataFireAndForget: Checking *DT response...");
    rv = m_HandleMessageHexByte(&transmissionResponseLen, MU_TRANSMISSION_RESPONSE_LEN, MU_TRANSMISSION_RESPONSE_PREFIX);

    if (rv != MU_Modem_Error::Ok) {
         MU_DEBUG_PRINTF(" Failed. err=%d. Rx: '%.*s'\n", (int)rv, m_rxIdx, m_rxMessage); // This format is fine
        return rv; // Parsing failed or wrong prefix
    }
    if (transmissionResponseLen != len)
    {
         MU_DEBUG_PRINTF("[MU_Modem] TransmitDataFireAndForget: Length mismatch in *DT response. Sent %u, Acked %u\n", len, transmissionResponseLen);
        return MU_Modem_Error::Fail; // Indicate failure due to length mismatch
    }

    MU_DEBUG_PRINTF(" *DT response OK (len=%u). Returning immediately.\n", transmissionResponseLen);
...
MU_Modem_Error MU_Modem::CheckCarrierSense()
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] CheckCarrierSense: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] CheckCarrierSense: Sending command...");
    m_WriteString(F(MU_GET_CHANNEL_STATUS_STRING)); // "@CS\r\n"
    MU_DEBUG_PRINTLN(" Sent.");

    // Wait for the response (*CS=EN or *CS=DI)
    // This should be very fast.
    MU_DEBUG_PRINTLN("[MU_Modem] CheckCarrierSense: Waiting for *CS response...");
    MU_Modem_Error rv = m_WaitCmdResponse(50); // Use a short timeout
    MU_DEBUG_PRINTF("[MU_Modem] CheckCarrierSense: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv != MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] CheckCarrierSense: Failed waiting for *CS response.");
    ...
    if (m_rxIdx == MU_CHANNEL_STATUS_RESPONSE_LEN) {
        if (strncmp((const char*)m_rxMessage, MU_CHANNEL_STATUS_OK_RESPONSE, MU_CHANNEL_STATUS_RESPONSE_LEN) == 0) {
            MU_DEBUG_PRINTLN("[MU_Modem] CheckCarrierSense: Channel is clear (*CS=EN).");
            return MU_Modem_Error::Ok;
        } else if (strncmp((const char*)m_rxMessage, MU_CHANNEL_STATUS_BUSY_RESPONSE, MU_CHANNEL_STATUS_RESPONSE_LEN) == 0) {
            MU_DEBUG_PRINTLN("[MU_Modem] CheckCarrierSense: Channel is busy (*CS=DI).");
            return MU_Modem_Error::FailLbt;
        }
    }

    MU_DEBUG_PRINTF("[MU_Modem] CheckCarrierSense: Unexpected response '%.*s'.\n", m_rxIdx, m_rxMessage);
...
MU_Modem_Error MU_Modem::SetBaudRate(uint32_t baudRate, bool saveValue)
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] SetBaudRate: Busy");
        return MU_Modem_Error::Busy;
    }
    ...
    uint8_t baudCode;
    switch (baudRate) {
        case 1200:   baudCode = 12; break;
        case 2400:   baudCode = 24; break;
        case 4800:   baudCode = 48; break;
        case 9600:   baudCode = 96; break;
        case 19200:  baudCode = 19; break;
        case 38400:  baudCode = 38; break;
        case 57600:  baudCode = 57; break;
        default:
            MU_DEBUG_PRINTF("[MU_Modem] SetBaudRate: Invalid baud rate %lu\n", baudRate);
            return MU_Modem_Error::InvalidArg;
    }

    MU_DEBUG_PRINTF("[MU_Modem] SetBaudRate: Sending command for %lu bps (code %u)...", baudRate, baudCode);
    ...
    MU_DEBUG_PRINTLN("[MU_Modem] SetBaudRate: Waiting for *BR response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SetBaudRate: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv != MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] SetBaudRate: Failed waiting for *BR response.");
        return rv; // Return Timeout or Parse Error
    }
    ...
    {
        MU_DEBUG_PRINTF("[MU_Modem] SetBaudRate: Unexpected response format '%.*s'.\n", m_rxIdx, m_rxMessage);
        return MU_Modem_Error::Fail;
    }
    ...
    {
        MU_DEBUG_PRINTF("[MU_Modem] SetBaudRate: Response code mismatch. Sent %u, Got %u.\n", baudCode, responseCode);
        return MU_Modem_Error::Fail;
    }

    // --- Success! ---
    // The modem will switch its baud rate immediately. The caller is now responsible for reconfiguring the host UART.
    MU_DEBUG_PRINTF("[MU_Modem] SetBaudRate: Success (*BR=%u). Caller must now reconfigure host UART to %lu bps.\n", responseCode, baudRate);
...
MU_Modem_Error MU_Modem::TransmitDataWithRoute(const uint8_t* pRouteInfo, uint8_t numNodes, const uint8_t *pMsg, uint8_t len, bool requestAck, bool outputToRelays)
{
    if (!pRouteInfo || numNodes == 0 || numNodes > MU_MAX_ROUTE_NODES || !pMsg || len == 0 || len > MU_MAX_PAYLOAD_LEN) {
        MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataWithRoute: Invalid arguments.");
        return MU_Modem_Error::InvalidArg;
    }

     if (m_asyncExpectedResponse != MU_Modem_Response::Idle) {
         MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataWithRoute: Busy.");
         return MU_Modem_Error::Busy;
     }

    MU_DEBUG_PRINT("[MU_Modem] TransmitDataWithRoute: Sending command...");
    m_WriteString(F(MU_TRANSMISSION_PREFIX_STRING)); // "@DT"
    char lenStr[3];
    snprintf(lenStr, sizeof(lenStr), "%02X", len);
    m_WriteString(lenStr);
    ...
    MU_DEBUG_PRINTLN(" Sent.");
     ...
     MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataWithRoute: Waiting for *DT response...");
     MU_Modem_Error rv = m_WaitCmdResponse();
      MU_DEBUG_PRINTF("[MU_Modem] TransmitDataWithRoute: m_WaitCmdResponse (DT) returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;

    uint8_t transmissionResponseLen{};
    rv = m_HandleMessageHexByte(&transmissionResponseLen, MU_TRANSMISSION_RESPONSE_LEN, MU_TRANSMISSION_RESPONSE_PREFIX);
    if (rv != MU_Modem_Error::Ok) return rv;
    if (transmissionResponseLen != len) return MU_Modem_Error::Fail;
    ...
    if (lbt_check_rv == MU_Modem_Error::Ok) {
     ...
     if (requestAck) {
         ...
         MU_DEBUG_PRINTLN("[MU_Modem] TransmitDataWithRoute: Waiting for ACK response...");
         ...
         if (rv == MU_Modem_Error::Ok) {
             ...
             if (m_rxIdx == 6 && strncmp("*DR=00", (char*)m_rxMessage, 6) == 0) {
                 MU_DEBUG_PRINTLN(" ACK response OK (*DR=00).");
                 rv = MU_Modem_Error::Ok; // Final result is OK
             } else {
                  MU_DEBUG_PRINTF("[MU_Modem] TransmitDataWithRoute: Unexpected ACK response '%.*s'.\n", m_rxIdx, m_rxMessage);
                  rv = MU_Modem_Error::Fail; // Unexpected ACK response
             }
         } else {
             MU_DEBUG_PRINTF("[MU_Modem] TransmitDataWithRoute: Failed waiting for ACK response (Error: %d).\n", (int)rv);
...
{
    if (!pChannel) return MU_Modem_Error::InvalidArg;

    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
         MU_DEBUG_PRINTLN("[MU_Modem] GetChannel: Busy");
        return MU_Modem_Error::Busy;
    }

     MU_DEBUG_PRINT("[MU_Modem] GetChannel: Sending command...");
    m_WriteString(F(MU_GET_CHANNEL_STRING));
    MU_DEBUG_PRINTLN(" Sent.");


    MU_DEBUG_PRINTLN("[MU_Modem] GetChannel: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] GetChannel: m_WaitCmdResponse returned %d\n", (int)rv);


    if (rv == MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINT("[MU_Modem] GetChannel: Handling response...");
        rv = m_HandleMessageHexByte(pChannel, MU_SET_CHANNEL_RESPONSE_LEN, MU_SET_CHANNEL_RESPONSE_PREFIX);
         MU_DEBUG_PRINTF(" Result %d, Channel 0x%02X\n", (int)rv, *pChannel);
    }
    return rv;
}
...
MU_Modem_Error MU_Modem::SetChannel(uint8_t channel, bool saveValue)
{
    ...
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
         MU_DEBUG_PRINTLN("[MU_Modem] SetChannel: Busy");
        return MU_Modem_Error::Busy;
    }

    // e.g. @CH0A\r\n or @CH45/W\r\n
    MU_DEBUG_PRINTF("[MU_Modem] SetChannel: Sending command (@CH%02X%s)...", channel, saveValue ? "/W" : "");
    ...
    // Wait for the first response (*WR=PS if /W, otherwise *CH=XX)
     MU_DEBUG_PRINTLN("[MU_Modem] SetChannel: Waiting for first response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SetChannel: m_WaitCmdResponse (1st) returned %d\n", (int)rv);
    ...
    if (saveValue)
    {
        MU_DEBUG_PRINT("[MU_Modem] SetChannel: Handling *WR response...");
        rv = m_HandleMessage_WR(); // Check if response is *WR=PS
        MU_DEBUG_PRINTF(" Result %d\n", (int)rv);

        if (rv == MU_Modem_Error::Ok)
        {
             MU_DEBUG_PRINTLN("[MU_Modem] SetChannel: Waiting for second response (*CH)...");
            rv = m_WaitCmdResponse(); // Wait for the actual *CH=XX response
             MU_DEBUG_PRINTF("[MU_Modem] SetChannel: m_WaitCmdResponse (2nd) returned %d\n", (int)rv);
            if (rv != MU_Modem_Error::Ok) {
                return rv; // Timeout or parse error waiting for *CH
            }
        } else {
             // Didn't get *WR=PS after sending /W command
             MU_DEBUG_PRINTLN("[MU_Modem] SetChannel: Did not receive *WR=PS after /W");
    ...
    {
        MU_DEBUG_PRINT("[MU_Modem] SetChannel: Handling *CH response...");
    ...
    {
        MU_DEBUG_PRINTF("[MU_Modem] SetChannel: Response mismatch. Sent %u, Acked %u\n", channel, channelResponse);
        rv = MU_Modem_Error::Fail;
    }
    ...
    if (rv == MU_Modem_Error::Ok && saveValue) {
        MU_DEBUG_PRINTLN("[MU_Modem] SetChannel: Re-enabling RSSI after saving...");
        ...
        if (rssiErr != MU_Modem_Error::Ok) {
             ...
             MU_DEBUG_PRINTLN("[MU_Modem] SetChannel: RSSI re-enabled OK.");
        }
    }


    MU_DEBUG_PRINTF("[MU_Modem] SetChannel: Final result: %d\n", (int)rv);
    return rv;
}
...
{
    if (!pGI) return MU_Modem_Error::InvalidArg;
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle) return MU_Modem_Error::Busy;
...
MU_Modem_Error MU_Modem::SetGroupID(uint8_t gi, bool saveValue)
{
    ...
    if (rv == MU_Modem_Error::Ok) {
        MU_Modem_Error handleRv = m_HandleMessageHexByte(&giResponse, MU_SET_GROUP_RESPONSE_LEN, MU_SET_GROUP_RESPONSE_PREFIX);
         if (handleRv != MU_Modem_Error::Ok) rv = handleRv;
    }
...
{
     if (!pDI) return MU_Modem_Error::InvalidArg;
     if (m_asyncExpectedResponse != MU_Modem_Response::Idle) return MU_Modem_Error::Busy;
...
{
    if (!pEI) return MU_Modem_Error::InvalidArg;
     if (m_asyncExpectedResponse != MU_Modem_Response::Idle) return MU_Modem_Error::Busy;
...
{
    if (!pUI) return MU_Modem_Error::InvalidArg;
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
         MU_DEBUG_PRINTLN("[MU_Modem] GetUserID: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] GetUserID: Sending command...");
    m_WriteString(F(MU_GET_USER_ID_STRING));
    MU_DEBUG_PRINTLN(" Sent.");


     MU_DEBUG_PRINTLN("[MU_Modem] GetUserID: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] GetUserID: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv == MU_Modem_Error::Ok)
    {
         MU_DEBUG_PRINT("[MU_Modem] GetUserID: Handling response...");
        ...
        {
             MU_DEBUG_PRINTF("[MU_Modem] GetUserID: Format error. Len=%u, Expected=%zu, Prefix Match=%d\n", m_rxIdx, MU_GET_USER_ID_RESPONSE_LEN, strncmp(MU_GET_USER_ID_RESPONSE_PREFIX, (char *)&m_rxMessage[0], strlen(MU_GET_USER_ID_RESPONSE_PREFIX)) == 0);
            return MU_Modem_Error::Fail;
        }
        ...
        {
            *pUI = (uint16_t)value;
             MU_DEBUG_PRINTF(" OK. UserID=0x%04X\n", *pUI);
            rv = MU_Modem_Error::Ok;
        } else {
             MU_DEBUG_PRINTLN(" Hex parse failed.");
             rv = MU_Modem_Error::Fail;
        }
    }
    return rv;
...
{
     if (!pRssi) return MU_Modem_Error::InvalidArg;
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] GetRssiCurrentChannel: Busy");
        return MU_Modem_Error::Busy;
    }

     MU_DEBUG_PRINT("[MU_Modem] GetRssiCurrentChannel: Sending command...");
    m_WriteString(F(MU_GET_RSSI_CURRENT_CHANNEL_STRING));
     MU_DEBUG_PRINTLN(" Sent.");


     MU_DEBUG_PRINTLN("[MU_Modem] GetRssiCurrentChannel: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] GetRssiCurrentChannel: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv == MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINT("[MU_Modem] GetRssiCurrentChannel: Handling response...");
        rv = m_HandleMessage_RA(pRssi); // Use the dedicated handler
         MU_DEBUG_PRINTF(" Result %d, RSSI=%d\n", (int)rv, *pRssi);
    } else {
...
{
    if (!pRssiBuffer || !pNumRssiValues) return MU_Modem_Error::InvalidArg;

    *pNumRssiValues = 0; // Initialize output

    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] GetAllChannelsRssi: Busy");
        return MU_Modem_Error::Busy;
    }
    ...
    MU_DEBUG_PRINT("[MU_Modem] GetAllChannelsRssi: Sending command...");
    m_WriteString(F(MU_GET_RSSI_ALL_CHANNELS_STRING));
    MU_DEBUG_PRINTLN(" Sent.");

    MU_DEBUG_PRINTLN("[MU_Modem] GetAllChannelsRssi: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse(2500); // Use a longer timeout as this command can take time
    MU_DEBUG_PRINTF("[MU_Modem] GetAllChannelsRssi: m_WaitCmdResponse returned %d\n", (int)rv);
    ...
    if (m_rxIdx != expectedResponseLen || strncmp(MU_GET_RSSI_ALL_CHANNELS_RESPONSE_PREFIX, (char*)m_rxMessage, prefixLen) != 0) {
        MU_DEBUG_PRINTF("[MU_Modem] GetAllChannelsRssi: Unexpected response. Len=%u (exp %zu), Prefix Match=%d\n", m_rxIdx, expectedResponseLen, strncmp(MU_GET_RSSI_ALL_CHANNELS_RESPONSE_PREFIX, (char*)m_rxMessage, prefixLen) == 0);
        return MU_Modem_Error::Fail;
    }
    ...
    for (size_t i = 0; i < expectedNumChannels; ++i) {
        ...
        if (s_ParseHex(pData + (i * 2), 2, &hexValue)) {
            ...
            MU_DEBUG_PRINTF("[MU_Modem] GetAllChannelsRssi: Failed to parse hex at index %zu.\n", i);
    ...
    *pNumRssiValues = expectedNumChannels;
    MU_DEBUG_PRINTF("[MU_Modem] GetAllChannelsRssi: Successfully parsed %u RSSI values.\n", *pNumRssiValues);
...
MU_Modem_Error MU_Modem::GetRssiCurrentChannelAsync()
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] GetRssiCurrentChannelAsync: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] GetRssiCurrentChannelAsync: Sending command...");
    m_WriteString(F(MU_GET_RSSI_CURRENT_CHANNEL_STRING));
    MU_DEBUG_PRINTLN(" Sent. Expecting async response.");
...
MU_Modem_Error MU_Modem::GetAllChannelsRssiAsync()
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] GetAllChannelsRssiAsync: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] GetAllChannelsRssiAsync: Sending command...");
    m_WriteString(F(MU_GET_RSSI_ALL_CHANNELS_STRING));
    MU_DEBUG_PRINTLN(" Sent. Expecting async response.");
...
MU_Modem_Error MU_Modem::SetRouteInfo(const uint8_t* pRouteInfo, uint8_t numNodes, bool saveValue)
{
    if (!pRouteInfo || numNodes == 0 || numNodes > MU_MAX_ROUTE_NODES) {
        MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfo: Invalid arguments.");
        return MU_Modem_Error::InvalidArg;
    }

    if (m_asyncExpectedResponse != MU_Modem_Response::Idle) {
        MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfo: Busy.");
        return MU_Modem_Error::Busy;
    }
    ...
    MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfo: Sending command '%s'...", cmdBuffer);
    m_WriteString(cmdBuffer);
    MU_DEBUG_PRINTLN(" Sent.");

    // Wait for and process response (including *WR handling for saveValue)
    MU_Modem_Error rv = m_WaitCmdResponse();
    MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfo: m_WaitCmdResponse (1st) returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;

    if (saveValue) {
        MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfo: Handling *WR response...");
        rv = m_HandleMessage_WR();
        MU_DEBUG_PRINTF(" Result %d\n", (int)rv);
        if (rv == MU_Modem_Error::Ok) {
            MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfo: Waiting for *RT response...");
            rv = m_WaitCmdResponse();
            MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfo: m_WaitCmdResponse (2nd) returned %d\n", (int)rv);
            if (rv != MU_Modem_Error::Ok) return rv;
        } else {
             MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfo: Did not receive *WR=PS after /W");
            return MU_Modem_Error::Fail;
        }
    }
    ...
    if (rv == MU_Modem_Error::Ok) {
        MU_DEBUG_PRINT("[MU_Modem] SetRouteInfo: Handling *RT response...");
        ...
        MU_DEBUG_PRINTF(" Result %d, NumNodes %u\n", (int)handleRv, responseNumNodes);

        if (handleRv != MU_Modem_Error::Ok) {
            rv = handleRv; // Parse failed
        } else if (responseNumNodes != numNodes) {
             MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfo: Node count mismatch. Sent %u, Acked %u\n", numNodes, responseNumNodes);
            ...
            if (memcmp(pRouteInfo, responseRoute, numNodes) != 0) {
                 MU_DEBUG_PRINTLN("[MU_Modem] SetRouteInfo: Route content mismatch.");
                 // Log detailed mismatch content if necessary
                rv = MU_Modem_Error::Fail;
            } else {
                 MU_DEBUG_PRINTLN(" Route set OK.");
                 // Update internal state on success if needed
                 // memcpy(m_currentRouteInfo, responseRoute, responseNumNodes);
                 // m_currentNumNodes = responseNumNodes;
            }
        }
    }
     MU_DEBUG_PRINTF("[MU_Modem] SetRouteInfo: Final result: %d\n", (int)rv);
    return rv;
}
...
MU_Modem_Error MU_Modem::GetRouteInfo(uint8_t* pRouteInfoBuffer, size_t bufferSize, uint8_t* pNumNodes)
{
     if (!pRouteInfoBuffer || bufferSize == 0 || !pNumNodes) {
         MU_DEBUG_PRINTLN("[MU_Modem] GetRouteInfo: Invalid arguments.");
         return MU_Modem_Error::InvalidArg;
     }
     ...
     *pNumNodes = 0; // Initialize output
     ...
     if (m_asyncExpectedResponse != MU_Modem_Response::Idle) {
         MU_DEBUG_PRINTLN("[MU_Modem] GetRouteInfo: Busy.");
         return MU_Modem_Error::Busy;
     }

     MU_DEBUG_PRINT("[MU_Modem] GetRouteInfo: Sending command (@RT)...");
     m_WriteString(F(MU_GET_ROUTE_STRING));
     MU_DEBUG_PRINTLN(" Sent.");

     MU_DEBUG_PRINTLN("[MU_Modem] GetRouteInfo: Waiting for response...");
     MU_Modem_Error rv = m_WaitCmdResponse();
     MU_DEBUG_PRINTF("[MU_Modem] GetRouteInfo: m_WaitCmdResponse returned %d\n", (int)rv);

     if (rv == MU_Modem_Error::Ok) {
         MU_DEBUG_PRINT("[MU_Modem] GetRouteInfo: Handling *RT response...");
...
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle) {
        MU_DEBUG_PRINTLN("[MU_Modem] ClearRouteInfo: Busy.");
        return MU_Modem_Error::Busy;
    }
    ...
    MU_DEBUG_PRINTF("[MU_Modem] ClearRouteInfo: Sending command '%s'...", cmdBuffer);
    m_WriteString(cmdBuffer);
    MU_DEBUG_PRINTLN(" Sent.");

    // Wait for and process response (including *WR handling for saveValue)
    MU_Modem_Error rv = m_WaitCmdResponse();
     MU_DEBUG_PRINTF("[MU_Modem] ClearRouteInfo: m_WaitCmdResponse (1st) returned %d\n", (int)rv);
    if (rv != MU_Modem_Error::Ok) return rv;

    if (saveValue) {
        MU_DEBUG_PRINTLN("[MU_Modem] ClearRouteInfo: Handling *WR response...");
        rv = m_HandleMessage_WR();
         MU_DEBUG_PRINTF(" Result %d\n", (int)rv);
        if (rv == MU_Modem_Error::Ok) {
             MU_DEBUG_PRINTLN("[MU_Modem] ClearRouteInfo: Waiting for *RT response...");
            rv = m_WaitCmdResponse();
             MU_DEBUG_PRINTF("[MU_Modem] ClearRouteInfo: m_WaitCmdResponse (2nd) returned %d\n", (int)rv);
            if (rv != MU_Modem_Error::Ok) return rv;
        } else {
             MU_DEBUG_PRINTLN("[MU_Modem] ClearRouteInfo: Did not receive *WR=PS after /W");
            return MU_Modem_Error::Fail;
        }
    }
    ...
    if (rv == MU_Modem_Error::Ok) {
        ...
        MU_DEBUG_PRINT("[MU_Modem] ClearRouteInfo: Handling *RT response...");
        if (m_rxIdx != expectedLen || strncmp(expectedResponse, (char*)m_rxMessage, expectedLen) != 0)
        {
             MU_DEBUG_PRINTF("[MU_Modem] ClearRouteInfo: Response mismatch. Expected '*RT=NA', Got '%.*s'\n", m_rxIdx, m_rxMessage);
            rv = MU_Modem_Error::Fail;
        } else {
             MU_DEBUG_PRINTLN(" Route cleared OK (*RT=NA).");
             // Update internal state if needed
             // m_currentNumNodes = 0;
        }
    }
     MU_DEBUG_PRINTF("[MU_Modem] ClearRouteInfo: Final result: %d\n", (int)rv);
    return rv;
}

MU_Modem_Error MU_Modem::GetPower(uint8_t *pPower)
{
    if (!pPower) return MU_Modem_Error::InvalidArg;
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle) return MU_Modem_Error::Busy;
...
{
    if (!pSerialNumber) return MU_Modem_Error::InvalidArg;
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
        MU_DEBUG_PRINTLN("[MU_Modem] GetSerialNumber: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] GetSerialNumber: Sending command...");
    m_WriteString(F(MU_GET_SERIAL_NUMBER_STRING));
     MU_DEBUG_PRINTLN(" Sent.");


     MU_DEBUG_PRINTLN("[MU_Modem] GetSerialNumber: Waiting for response...");
    MU_Modem_Error rv = m_WaitCmdResponse();
     MU_DEBUG_PRINTF("[MU_Modem] GetSerialNumber: m_WaitCmdResponse returned %d\n", (int)rv);

    if (rv == MU_Modem_Error::Ok)
    {
        MU_DEBUG_PRINT("[MU_Modem] GetSerialNumber: Handling response...");
        rv = m_HandleMessage_SN(pSerialNumber);
        MU_DEBUG_PRINTF(" Result %d, SN %lu\n", (int)rv, *pSerialNumber);
    } else {
...
MU_Modem_Error MU_Modem::GetSerialNumberAsync()
{
    if (m_asyncExpectedResponse != MU_Modem_Response::Idle)
    {
         MU_DEBUG_PRINTLN("[MU_Modem] GetSerialNumberAsync: Busy");
        return MU_Modem_Error::Busy;
    }

    MU_DEBUG_PRINT("[MU_Modem] GetSerialNumberAsync: Sending command...");
    m_WriteString(F(MU_GET_SERIAL_NUMBER_STRING));
    MU_DEBUG_PRINTLN(" Sent. Expecting async response.");
...
{
    if (!ppData || !len) return MU_Modem_Error::InvalidArg;
    ...
    if (m_drMessagePresent)
    {
        ...
        *len = m_drMessageLen;
        ...
        MU_DEBUG_PRINTF("[MU_Modem] GetPacket: Packet available. Len=%u\n", *len);
        return MU_Modem_Error::Ok;
    }
    else {
        MU_DEBUG_PRINTLN("[MU_Modem] GetPacket: No packet available.");
        *ppData = nullptr;
        *len = 0;
        return MU_Modem_Error::Fail;
    }
...
void MU_Modem::Work()
{
    ...
    case MU_Modem_CmdState::FinishedCmdResponse:
        ...
        if (m_asyncExpectedResponse != MU_Modem_Response::Idle) {
             MU_DEBUG_PRINTF("[MU_Modem] Work: Finished CMD response, dispatching async type %d\n", (int)m_asyncExpectedResponse);
    ...
    case MU_Modem_CmdState::FinishedDrResponse:
        // A data packet (*DR=... or *DS=...) was received.
         MU_DEBUG_PRINTF("[MU_Modem] Work: Finished DR response. Len=%u, RSSI=%d, RouteNodes=%u\n", m_drMessageLen, m_lastRxRSSI, m_drNumRouteNodes);
        if (m_pCallback)
        {
            ...
            // No callback set, but m_drMessagePresent is true. User can check with HasPacket().
             MU_DEBUG_PRINTLN("[MU_Modem] Work: No callback set for DR, use HasPacket/GetPacket.");
        }
...
uint8_t MU_Modem::m_ReadByte()
{
    ...
    if (rcv_int != -1) {
        if (rcv >= 32 && rcv <= 126) { // Printable char
            MU_DEBUG_WRITE(rcv);
        } else if (rcv == '\r') { // Carriage Return
            MU_DEBUG_PRINT("<CR>");
        } else if (rcv == '\n') { // Line Feed
        ...
        switch (m_parserState)
        {
            ...
            case MU_Modem_ParserState::ReadCmdParam:
                if (currentByte == '=') {
                    ...
                    } else { // Should not happen if state logic is correct
                    ...
                    // For now, assume most commands have '=', go to ReadCmdUntilCR state.
                     ...
                     if (m_rxIdx >= sizeof(m_rxMessage)) {
                         ...
                         m_FlushGarbage(); return MU_Modem_CmdState::Overflow;
                     }
             ...
             case MU_Modem_ParserState::ReadDsRSSI: // Expecting 2 hex digits for RSSI after *DS=
                 ...
                 } else if (m_rxIdx > 6) { // Should not happen
                     ...
                     if (isxdigit(m_rxMessage[4]) && isxdigit(m_rxMessage[5]) && s_ParseHex(&m_rxMessage[4], 2, &msgLen)) {
                         if (msgLen <= sizeof(m_drMessage)) { // Check if length fits buffer
                            ...
                            m_parserState = MU_Modem_ParserState::RadioDrPayload;
                             ...
                             m_FlushGarbage(); return MU_Modem_CmdState::Overflow; // Or maybe Garbage?
                         }
                     } else {
...
{
    if (!pDestBuffer || bufferSize == 0 || !pNumNodes) return MU_Modem_Error::InvalidArg;
    ...
    *pNumNodes = 0; // Initialize
    ...
    if (m_rxIdx < prefixLen || strncmp(MU_SET_ROUTE_RESPONSE_PREFIX, (const char*)m_rxMessage, prefixLen) != 0) {
        MU_DEBUG_PRINTF(" Prefix mismatch or too short. Expected '%s...', Got '%.*s'\n", MU_SET_ROUTE_RESPONSE_PREFIX, m_rxIdx, m_rxMessage);
        return MU_Modem_Error::Fail;
    }

    // Check for "NA"
    if (m_rxIdx == MU_SET_ROUTE_RESPONSE_LEN && strncmp((const char*)m_rxMessage, "*RT=NA", MU_SET_ROUTE_RESPONSE_LEN) == 0) {
         MU_DEBUG_PRINTLN(" Route is Not Available (NA).");
         *pNumNodes = 0;
         return MU_Modem_Error::Ok;
    }
    ...
    while (pStart < pEnd && nodeCount < bufferSize && nodeCount < MU_MAX_ROUTE_NODES) {
        ...
        if (hexLen != 2) { // Expect 2 hex digits per node
             MU_DEBUG_PRINTF(" Invalid hex length (%zu) at node %u.\n", hexLen, nodeCount);
            return MU_Modem_Error::Fail;
        }
        ...
        if (s_ParseHex((const uint8_t*)pStart, 2, &hexValue)) {
            pDestBuffer[nodeCount++] = (uint8_t)hexValue;
             ...
             MU_DEBUG_PRINTF(" Invalid hex value '%.*s' at node %u.\n", 2, pStart, nodeCount);
            return MU_Modem_Error::Fail;
        }
     ...
     if (pStart < pEnd) { // Check if loop terminated due to buffer full or max nodes reached
         MU_DEBUG_PRINTLN(" Buffer too small or too many nodes in response.");
...
MU_Modem_Error MU_Modem::m_DispatchCmdResponseAsync()
{
    ...
    MU_DEBUG_PRINTF("[MU_Modem] m_DispatchCmdResponseAsync: Processing response for expected type %d\n", (int)expected);
    ...
    switch (expected)
    {
        ...
        case MU_Modem_Response::SerialNumber:
             MU_DEBUG_PRINT("[MU_Modem] m_DispatchCmdResponseAsync: Handling SerialNumber response...");
            uint32_t sn_val;
            err = m_HandleMessage_SN(&sn_val); // Parse *SN=... response
            value = static_cast<int32_t>(sn_val); // Store SN in value
            MU_DEBUG_PRINTF(" Result err=%d, SN=%lu\n", (int)err, sn_val);
            break;

        case MU_Modem_Response::RssiCurrentChannel:
            MU_DEBUG_PRINT("[MU_Modem] m_DispatchCmdResponseAsync: Handling RssiCurrentChannel response...");
            int16_t rssi_val;
            err = m_HandleMessage_RA(&rssi_val); // Parse *RA=... response
            value = static_cast<int32_t>(rssi_val); // Store RSSI in value
            MU_DEBUG_PRINTF(" Result err=%d, RSSI=%d\n", (int)err, rssi_val);
            break;

        case MU_Modem_Response::RssiAllChannels:
            MU_DEBUG_PRINT("[MU_Modem] m_DispatchCmdResponseAsync: Handling RssiAllChannels response...");
            ...
            // We just validate the prefix and length here to determine the error code.
            {
                ...
                if (m_rxIdx == expectedLen && strncmp(MU_GET_RSSI_ALL_CHANNELS_RESPONSE_PREFIX, (char*)m_rxMessage, strlen(MU_GET_RSSI_ALL_CHANNELS_RESPONSE_PREFIX)) == 0) {
                    ...
                    payloadLen = m_rxIdx;
                    MU_DEBUG_PRINTF(" OK. PayloadLen=%u\n", payloadLen);
                } else {
                    err = MU_Modem_Error::Fail;
                    MU_DEBUG_PRINTF(" Failed. Len/Prefix mismatch. Len=%u (exp %zu)\n", m_rxIdx, expectedLen);
                }
            }
            break;

         case MU_Modem_Response::GenericResponse: // Used by SendRawCommand
             MU_DEBUG_PRINT("[MU_Modem] m_DispatchCmdResponseAsync: Handling GenericResponse...");
    ...
    if (m_pCallback)
    {
        MU_DEBUG_PRINTF("[MU_Modem] m_DispatchCmdResponseAsync: Calling callback with Error=%d, Type=%d, Value=%ld, Len=%u\n", (int)err, (int)expected, value, payloadLen);
...
MU_Modem_Error MU_Modem::m_HandleMessage_WR()
{
    ...
    MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessage_WR: Expected '%s', got '%.*s' (len %u)\n", MU_WRITE_VALUE_RESPONSE_PREFIX, m_rxIdx, m_rxMessage, m_rxIdx);
    return MU_Modem_Error::Fail;
}
...
{
    if (!pValue || !responsePrefix) return MU_Modem_Error::InvalidArg;

    size_t prefixLen = strlen(responsePrefix);
    // Check total length and prefix length validity
    if (responseLen != prefixLen + 2) {
        MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessageHexByte Internal Error: responseLen (%u) != prefixLen (%zu) + 2\n", responseLen, prefixLen);
    ...
    {
         MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessageHexByte: Length mismatch. Expected %u, Got %u ('%.*s')\n", responseLen, m_rxIdx, m_rxMessage);
    ...
    {
         MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessageHexByte: Prefix mismatch. Expected '%s', Got '%.*s'\n", responsePrefix, prefixLen, m_rxMessage);
    ...
    {
        MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessageHexByte: Hex parse failed for '%c%c'\n", m_rxMessage[prefixLen], m_rxMessage[prefixLen+1]);
...
MU_Modem_Error MU_Modem::m_HandleMessage_SN(uint32_t *pSerialNumber)
{
    ...
    {
         MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessage_SN: Len/Prefix mismatch. Len=%u, Prefix Match=%d\n", m_rxIdx, strncmp(MU_GET_SERIAL_NUMBER_RESPONSE_PREFIX, (char *)&m_rxMessage[0], prefixLen)==0);
        return MU_Modem_Error::Fail;
    }
    ...
    MU_DEBUG_PRINTF("[MU_Modem] m_HandleMessage_SN: Failed to parse decimal SN part (len %u).\n", parseLen);
...
MU_Modem_Error MU_Modem::SendRawCommand(const char* command, char* responseBuffer, size_t bufferSize, uint32_t timeoutMs) {
    ...
    MU_DEBUG_PRINT("[MU_Modem] SendRawCommand: Sending raw command...");
    m_WriteString(command);
     MU_DEBUG_PRINTLN(" Sent.");

     MU_DEBUG_PRINTF("[MU_Modem] SendRawCommand: Waiting for response (%lu ms)...", timeoutMs);
    MU_Modem_Error rv = m_WaitCmdResponse(timeoutMs);
     MU_DEBUG_PRINTF(" m_WaitCmdResponse returned %d\n", (int)rv);
    ...
    if (rv == MU_Modem_Error::Ok) {
        ...
        if (m_rxIdx < bufferSize) {
            ...
            responseBuffer[m_rxIdx] = '\0'; // Null-terminate
             MU_DEBUG_PRINTF("[MU_Modem] SendRawCommand: Response received: %s\n", responseBuffer);
            return MU_Modem_Error::Ok;
        } else {
             MU_DEBUG_PRINTF("[MU_Modem] SendRawCommand: Response length (%u) exceeds buffer size (%zu).\n", m_rxIdx, bufferSize);
