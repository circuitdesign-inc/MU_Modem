  ...
  if (event.type == MU_Modem_Response::DataReceived)
  {
    if (event.error == MU_Modem_Error::Ok)
    {
      ...
      if (event.payloadLen > 0 && event.payloadLen <= sizeof(g_receivedPayload))
      {
        ...
        g_lastRssi = (int16_t)event.value;
...
void setup()
{
  ...
  Serial.println("\n--- 中継通信サンプル ---");
  Serial.print("この局のID: 0x");
  Serial.println(MY_EQUIPMENT_ID, HEX);
  ...
  Serial.print("チャンネルを 0x");
  Serial.print(COMMON_CHANNEL, HEX);
  ...
  Serial.print("グループIDを 0x");
  Serial.print(COMMON_GROUP_ID, HEX);
  Serial.println(" に設定しました。");
  ...
  Serial.print("自機IDを 0x");
  Serial.print(MY_EQUIPMENT_ID, HEX);
  Serial.println(" に設定しました。");
  ...
  if (MY_EQUIPMENT_ID == SOURCE_ID)
  {
    ...
    {
      Serial.print("ルート情報を [0x");
      Serial.print(RELAY_ID, HEX);
      Serial.print(", 0x");
