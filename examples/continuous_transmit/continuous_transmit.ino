 ...
 *
 * @details
 * このサンプルプログラムは、CheckCarrierSense() と TransmitDataAsync() を組み合わせて、
 ...
 * 4. チャンネルが空いていれば、forループで TransmitDataAsync() を連続で呼び出し、パケットを間髪入れずに送信します。
 ...
 * 一方、TransmitDataAsync()はコマンドをキューに入れ、モデムからの*DT応答を待たずにすぐに制御を返すため、この関数を連続で呼び出すことで、連続送信状態を効率的に保持できます。
 * 詳細はデータシートをご確認ください。
...
void loop()
{
  ...
  if (millis() - lastSendTime > 10000) // 10秒ごとに送信
  {
    ...
    // しかし、その直後に TransmitDataAsync() を呼び出すまでのわずかな時間で
    // 他の無線機が送信を開始すると、TransmitDataAsync() 内部のキャリアセンスで送信が失敗する可能性があります。
    //
    ...
    // b. その直後から、間髪入れずに TransmitDataAsync() で後続のデータを次々と送信する。
    ...
    if (cs_err == MU_Modem_Error::Ok)
    {
      ...
      const int NUM_PACKETS_TO_SEND = 100;
      // 非同期送信のために、パケットごとに独立したバッファを用意します。
      // これを行わないと、送信完了前に次のデータでバッファが上書きされ、データ化けが発生します。
      static char messageBuffers[NUM_PACKETS_TO_SEND][32];

      for (int i = 0; i < NUM_PACKETS_TO_SEND; i++)
      {
        // キューがいっぱいの場合は、空きができるまで待機します
        while (modem.isQueueFull())
        {
          modem.Work(); // 待機中もモデムの処理を進める必要があります
        }

        // バッファにデータをセット
        snprintf(messageBuffers[i], sizeof(messageBuffers[i]), "Cont Pkt #%d", i + 1);

        // TransmitDataAsync()を使用して、コマンドをキューに入れ、非同期に送信します。
        MU_Modem_Error tx_err = modem.TransmitDataAsync((const uint8_t *)messageBuffers[i], strlen(messageBuffers[i]));

        if (tx_err == MU_Modem_Error::Ok)
        {
          Serial.printf("   Packet %d: 送信コマンド受付成功 (\"%s\") [Queue: %d]\n", i + 1, messageBuffers[i], modem.getQueueCount());
        }
        else
        {
