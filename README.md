...
| `GND`  |      `GND`       | グランドを接続します。                                                                                                              |
| `TXD`  |  Arduinoの `RX`  | MUモデムの送信(TXD)をArduinoの受信(RX)に接続します。ロジックレベルはVCCに依存します。                                               |
| `RXD`  |  Arduinoの `TX`  | MUモデムの受信(RXD)をArduinoの送信(TX)に接続します。ロジックレベルはVCCに依存します。                                               |
...
|  MU端子  |     接続先 (Arduino)      | 説明                                                                                                                              |
| :------: | :-----------------------: | :------------------------------------------------------------------------------------------------------------------------------ |
|  `RTS`   | Arduinoのデジタル入力ピン | ハードウェアフロー制御用の出力です。モデムがデータ受信可能かを示します。使用しない場合はオープン（未接続）で構いません。           |
|  `CTS`   | Arduinoのデジタル出力ピン | ハードウェアフロー制御用の入力です。モデムにデータ送信を許可/禁止します。**この機能を使用しない場合は、`GND`に接続してください**（HIGHのままだとモデムが送信を停止します）。 |
...
void setup() {
  ...
  modem.SetDestinationID(0x00, false);  // 目的局IDを0に設定(0x00はブロードキャスト)
}
